...
export async function parseHtmlFile(changeKey = 3) {
  const doc = await fs.promises.readFile(
    "files/input/Praise-Doc_1.html",
    "utf-8",
  );
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    // Format chords
    depth.each((index, element) => {
      if (chordValidator(toText(extractNodes($(element))))) {
        const cleanedChords = chordCleaner(
          extractNodes($(element)),
          splitChords(toText(extractNodes($(element)))),
        );
        // console.log("🚀 ~ parseHtmlFile ~ cleanedChords:", cleanedChords);
        // console.log(
        //   "Target: ",
        //   extractNodes($(element)),
        //   "Split:",
        ...
        extractNodes($(element)).forEach((node, index) => {
          replaceNode($(element), index, cleanedChords[index]);
        });
      }
    });
    ...
    // Transpose chords
    depth.each((index, element) => {
      // console.log("Element: ", extractNodes($(element)), index);
      if (chordValidator(toText(extractNodes($(element))))) {
        extractNodes($(element)).forEach((node, idx) => {
          try {
            if (node && node.replace(/[\s\u2800]+/g, "") !== "") {
              replaceNode(
                ...
                idx,
                transpose(node)
                  .up(changeKey) // Transpose
                  .toString()
                  .replace(/b/g, "\u266D"),
              );
            } else {
              replaceNode($(element), idx, node.replace(/b/g, "\u266D"));
            }
          } catch (err) {
  ...
  await fs.promises.writeFile(
    "files/output/Praise-Doc_1.html",
    $.html(),
    "utf-8",
  );

  // console.log(transpose("D/F3").up(3).toString());
  // console.log(transpose("EsusE").up(3).toString());
  // console.log(
  //   chordValidator(
  //     "Cmaj13add2/F#Gsus2A#min11Db7add4Ebmaj9sus4/GAadd9F#maj7sus2/BCmin7Gbadd2A/C"
  //   )
  // );
}
